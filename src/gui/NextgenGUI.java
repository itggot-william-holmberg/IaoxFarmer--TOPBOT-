...
public class NextgenGUI {
	...
	private JComboBox comboBoxCombatAssignment;

	private JComboBox comboBoxAmountOfFood;
	...
	private void initialize() {
		...
		btnStart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(comboBoxCombatAssignment.getSelectedItem() != null){
					Data.COMBAT_ASSIGNMENT = (CombatAssignment) comboBoxCombatAssignment.getSelectedItem();
				}
				
				if(comboBoxAmountOfFood.getSelectedItem() != null){
					Data.amountOfFood = (int) comboBoxAmountOfFood.getSelectedItem();
				}
		...
		addButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				Assignment ass = sourceList.getSelectedValue();
				destmodel.addElement(new Task(ass, ass.getSkill(),
		...
		comboBoxCombatAssignment = new JComboBox();
		comboBoxCombatAssignment.setBounds(467, 49, 52, 27);
		frame.getContentPane().add(comboBoxCombatAssignment);
		
		comboBoxAmountOfFood = new JComboBox();
		comboBoxAmountOfFood.setBounds(467, 175, 52, 27);
		for(int i = 0; i<20; i++){
			comboBoxAmountOfFood.addItem(i);
		}
		frame.getContentPane().add(comboBoxAmountOfFood);
		
		Arrays.asList(CombatAssignment.values()).forEach(value -> {
			comboBoxCombatAssignment.addItem(value);
		});
	...
	public class IngredientListCellRenderer extends DefaultListCellRenderer {
		public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
			...
			if (value instanceof Task) {
				...
				setText("Task: " + ingredient.getLevelGoal() + ingredient.getAssignment());
			}
			return this;
		}
	}
